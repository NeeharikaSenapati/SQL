...
select account_type,sum(transaction_amount)  ,if(weekday(transaction_date)=4,'Friday','other') w_day
from bank_account_transaction bat join bank_account_details bad
on bat.Account_Number=bad.Account_Number
group by w_day,Account_type
...
select bad.account_type, sum(transaction_amount),if(Start_time like '%2020-03-13%','holiday','non holiday') as hn
from bank_account_details bad  join bank_account_transaction bat
on bad.Account_Number=bat.Account_Number
join bank_holidays 
group by Account_type,hn
 having account_type like '%CREDIT%';
...
select holiday,transcation_channel,if(holiday between '2020-03-01'and '2020-03-30','happy_holiday','no_holiday') as h
from bank_account_transaction bat
join bank_holidays bh 
group by transcation_channel,h;
# Question 4:
# 4) Calculate the Bank accrued interest with respect to their RECURRING DEPOSITS  for any deposits older than 30 days . 
select account_type,transaction_amount,Transaction_Date, transaction_amount*interest_rate as interest
from bank_account_transaction
join bank_interest_rate
group by interest,Transaction_amount
having account_type like '%RECURRING DEPOSITS%' and subdate(current_date(),interval 30 day)>transaction_date;
...
select bat.Account_Number,count(bat.transaction_amount) as ct,bad.account_type
from bank_account_details bad join bank_account_transaction bat
on bad.Account_Number=bat.Account_Number
group by account_type
having Account_type like 'Credit%' and ct>1; 
# Question 6:
...
on bad.Account_Number=bat.Account_Number
group by account_type
having Account_type like '%Add-on Credit Card%' and ct>=1; 
...
select bce.telephone,bc.customer_id,bc.customer_name
from bank_customer bc join bank_customer_export bce
on bc.telephone=bce.telephone;
