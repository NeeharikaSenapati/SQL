...
start transaction;
update account_details set balance = balance -1000 where first_name = 'Monica';
update account_details set balance = balance +1000 where first_name = 'Prter';
rollback ;
select * from account_details;
...
(select employee_id, first_name, job_id, salary from employee_details where first_name = '%l');
 select * from location_details;
...
( select e.employee_id,e.department_id,e.manager_id from employee_details e join department_details d using (employee_id) );	
