...
select bc.customer_name,avg(Balance_amount),ba.customer_id
...
select distinct customer_id,ba.account_number,account_type,balance_amount,
if(Balance_amount=0 and ba.account_type like '%credit%',Transaction_amount,balance_amount) balance
...
select bt.account_number,ba.balance_amount,bt.transaction_amount,bt.transaction_date
from bank_account_details ba join bank_account_transaction bt
on ba.Account_Number=bt.Account_Number
...
from bank_account_details ba join bank_account_transaction bt
on ba.Account_Number=bt.Account_Number
where Transaction_Date between '2020-01-01' and '2020-03-30';
...
select account_number,customer_message,event
from bank_account_details bad cross join bank_customer_messages  
where Account_type like 'SAVINGS%' and event like 'Adhoc';
...
select bad.Account_Number,bad.Account_type,bard.Account_Number,bard.Account_type
from bank_account_details bad join bank_account_relationship_details bard
...
select bad.Account_Number,bad.Account_type,bad.Balance_amount,bar.Account_Number,bar.Account_type,bar.Linking_Account_Number,bat.transaction_amount
...
select bad.account_type,sum(Transaction_amount)
...
# 10) Display every account's total no of transactions for every year and each month.
 select count(transaction_date),bat.Account_Number,bat.Transaction_Date,year(transaction_date)as year,month(Transaction_Date) as month
 from  bank_account_details bad join bank_account_transaction bat
 on bad.Account_Number=bat.Account_Number
 group by year,month;
...
br.account_number as recurring_deposit_account_number, br.account_type as recurring_deposit_account_type, count(linking_account_number)
from bank_account_details ba join bank_account_relationship_details br 
on ba.account_number = br.linking_account_number join bank_account_transaction bat
...
select count(transaction_amount),Account_type
from bank_account_relationship_details bar join bank_account_transaction bat
on bar.Account_Number=bat.Account_Number
where Account_type in ('Credit' 'Add on')
group by Account_type;