...
alter table bank_inventory change Geo_Location Geo_Location varchar(30);
desc bank_inventory;
# Question 2:
# 2) Update  estimated_sale_price of bank_inventory table with an increase of 15%  when the quantity of product is more than 4.
update bank_inventory set estimated_sale_price=(estimated_sale_price+(estimated_sale_price*0.15))
where Quantity>4;
...
update bank_inventory set estimated_sale_price=(estimated_sale_price+(estimated_sale_price*0.10))
where Quantity=2;
select * from bank_inventory;

insert into bank_inventory values
('DailCard',2,380,8500,9000,NULL);
